  ...
  <div id="app">
    <div class="main-wrapper main-wrapper-1">
                            ...
                            while($result=mysqli_fetch_array($sql)){
                              ?>
                                <tr>
                                  ...
                                  <td><a href="deletecancer.php?table=income&where=in_id=<?php echo $result['in_id'] ?>&location=income.php" class="btn btn-danger">Delete</a></td>
                                </tr>
                             <?php
                            }
                          }else{
      ...
      if(isset($_POST['btn'])){
        ...
        $amnt = $_POST['amount'];
        $date =  date('y-m-d');
        $sel=mysqli_query($con, "SELECT * FROM income WHERE source='$src'");      
        if(mysqli_num_rows($sel)>0){
          $resSel=mysqli_fetch_array($sel);
          $red = $resSel['in_id'];
          $up=mysqli_query($con, "UPDATE income SET amount=(amount+$amnt) WHERE user='$uid' AND in_id='$red'");
        }else{
        $sql = mysqli_query($con, "INSERT INTO income (source,amount,date,outgoing,user) VALUES ('$src','$amnt','$date','$amnt','$uid')");
        }
        if($up){
